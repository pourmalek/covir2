...
gen infectionsDIVIDEDbypopulation = TotINFMeRaA02S01 / 84297880000

gen infectionsDIVbypop01nov2021 = TotINFMeRaA02S01 / 84297880000 if date == td(01nov2021)

twoway ///
(line infectionsDIVIDEDbypopulation date, sort lcolor(black) lwidth(medium)) ///
, xtitle(Date) xlabel(#24, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%9.2fc) labsize(small))  ylabel(, labsize(small) angle(horizontal)) ///
ytitle(Total infections divided by population) title("COVID-19 total infections / by population, $country, IHME, reference scenario", size(medium)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(off) yscale(titlegap(2)) ///
subtitle("Estimated value on 01 November 2021: 0.9006356")
	
graph save "graph X COVID-19 total infections divided by population, reference scenario, $country, IHME.gph", replace
graph export "graph X COVID-19 total infections divided by population, reference scenario, $country, IHME.pdf", replace
