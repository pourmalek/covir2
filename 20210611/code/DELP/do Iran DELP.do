...
* OBJECTIVE: Get and prepare DELPHI data for Iran
***************************************************************************
...
if loc_grand_name == "Iran" & DayDeaMeRaA01S00 != ., legend(off) ///	   
...
if loc_grand_name == "Iran" & DayDeaMeRaA01S00 != ., ///	   
xtitle(Date) xlabel(, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%9.0fc) labsize(small))  ylabel(, labsize(small) angle(horizontal)) ///
...
graph save "graph 12 COVID-19 daily deaths and cases Iran, DELP.gph", replace
graph export "graph 12 COVID-19 daily deaths and cases Iran, DELP.pdf", replace
