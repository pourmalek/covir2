...
, xtitle(Date) xlabel(#28, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) ///
...
, xtitle(Date) xlabel(#17, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%9.0fc) labsize(small))  ylabel(, labsize(small) angle(horizontal)) ///
ytitle(Daily deaths) title("COVID-19 daily deaths, $country", size(medium)) /// 
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
legend(order(1 "JOHN" 2 "DELP" 3 "IHME" 4 "IHME excess" 5 "IMPE" 6 "SRIV") size(small) row(1)) ///
...
****
* daily deaths, 5 scenarios, IHME, 2021
...
graph save "graph 15 COVID-19 daily deaths, $country, 5 scenarios, IHME.gph", replace
graph export "graph 15 COVID-19 daily deaths, $country, 5 scenarios, IHME.pdf", replace
...
(line dai_inf_A02S01 date, sort lcolor(black)) ///
