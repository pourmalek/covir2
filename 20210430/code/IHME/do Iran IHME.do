...
// copy https://ihmecovid19storage.blob.core.windows.net/archive/2021-04-30/ihme-covid19.zip ihme-covid19.zip
...
* format of date variable has changed in this update
* to a mix of 1/1/2021 and 2020-02-04

gen which_date_format = ""

gen year = substr(date,1,4)

replace which_date_format = "2020" if year == "2020"
replace which_date_format = "2020" if year == "2021"

* now which_date_format = "2020" means like 2020-02-04 starting with 2020 or 2021
* and  which_date_format = "" means like 1/1/2021 or 1/10/2021 starting with month
drop year



rename date date_original
gen year = substr(date_original,1,4) if which_date_format == "2020"
gen month = substr(date_original,6,2) if which_date_format == "2020"
gen day = substr(date_original,9,2) if which_date_format == "2020"
egen date2 = concat(day month year) if which_date_format == "2020"
gen date = date(date2, "DMY", 2050) if which_date_format == "2020"
...
gen year = substr(date_original,-4,4) if which_date_format == ""
gen day = substr(date_original,-7,2) if which_date_format == ""
...
gen month = substr(date_original,1,2) if which_date_format == ""
...
egen date2 = concat(day month year) if which_date_format == ""
...
gen date = date1
replace date = date2 if date2 != . 
format date %tdDDMonCCYY
drop date1 date2 which_date_format
...
drop date_original v1  deaths_data_type ///
...
// not available in this update hosp_data_type
...
, xtitle(Date) xlabel(#18, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%9.5f) labsize(small))  ylabel(, labsize(small) angle(horizontal)) ///
...
graph save "graph 45 COVID-19 daily deaths and cases Iran, IHME.gph", replace
