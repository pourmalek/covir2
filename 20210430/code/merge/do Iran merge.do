...
* daily hospital-related outcomes, DELP, IHME, wo IMPE

twoway ///
(line DayHosMeRaA01S00V03 date, sort lcolor(red)  lwidth(thick)) /// 1 DELP Hospitalized (Daily Cases Active Hospitalized Mean DELP S0 V3)
(line DayVenMeRaA01S00V03 date, sort lcolor(red) lpattern(dash)  lwidth(thick)) /// 2 DELP Ventilated (Daily Cases Active Ventilated Mean DELP S0 V3)
(line DayHodMeRaA03S01V07 date, sort lcolor(magenta)) /// 3 IMPE Hosp demand (Daily hospital demand Mean A03 S01 V07)
...
ytitle(Daily outcomes) title("COVID-19 daily hospital-related outcomes Iran, median scenarios", size(medium)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
legend(order(1 "DELP Hospitalized" 2 "DELP Ventilated" 3 "IMPE Hosp demand" ///
4 "IMPE Hosp incidence" 5 "IMPE ICU demand" 6 "IMPE ICU incidence") rows(4) size(small)) ///
subtitle("without IHME in this update", size(small))
...
* daily hospital-related outcomes, DELP, wo IHME, IMPE, wo extremes (IMPE Hosp demand)

twoway ///
(line DayHosMeRaA01S00V03 date, sort lcolor(red)  lwidth(thick)) /// 1 DELP Hospitalized (Daily Cases Active Hospitalized Mean DELP S0 V3)
(line DayVenMeRaA01S00V03 date, sort lcolor(red) lpattern(dash)  lwidth(thick)) /// 2 DELP Ventilated (Daily Cases Active Ventilated Mean DELP S0 V3)
(line DayHoiMeRaA03S01V07 date, sort lcolor(magenta) lpattern(dash)) /// 3 IMPE Hosp incidence (Daily hospital incidence Mean A03 S01 V07)
(line DayIcdMeRaA03S01V07 date, sort lcolor(magenta) lpattern(dash_dot)) /// 4 IMPE ICU demand (Daily ICU demand Mean A03 S01 V07)
(line DayIciMeRaA03S01V07 date, sort lcolor(magenta) lpattern(longdash_shortdash)) /// 5 IMPE ICU incidence (Daily ICU incidence Mean A03 S01 V07)
if date >= td(30dec2019) & date <= td(23Aug2021) ///
, xtitle(Date) xlabel(#19, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%9.0fc) labsize(small))  ylabel(, labsize(small) angle(horizontal)) ///
ytitle(Daily outcomes) title("COVID-19 daily hospital-related outcomes Iran, median scenarios", size(medium)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
legend(order(1 "DELP Hospitalized" 2 "DELP Ventilated" ///
3 "IMPE Hosp incidence" 4 "IMPE ICU demand" 5 "IMPE ICU incidence") rows(3) size(small)) ///
subtitle("without extremes (IMPE Hosp demand), without IHME in this update", size(small))
...
* daily hospital-related outcomes, DELP, IHME, wo IMPE, wo extremes (IMPE Hosp demand), 2020-12-01 on 
...
legend(order(1 "DELP Hospitalized" 2 "DELP Ventilated" ///
3 "IMPE Hosp incidence" 4 "IMPE ICU demand" 5 "IMPE ICU incidence") rows(3) size(small)) ///
subtitle("after 2020-12-01, without extremes (IMPE Hosp demand), without IHME in this update", size(small))
