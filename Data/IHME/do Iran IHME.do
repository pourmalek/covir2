...
twoway ///
(line TotCasMeRaA02S01 date, sort lcolor(black) lwidth(vthick) lpattern(tight_dot)) ///
(line TotCasMeRaA02S02 date, sort lcolor(green)) ///
(line TotCasMeRaA02S03 date, sort lcolor(red)) ///
, xtitle(Date) xlabel(#17, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%12.0fc) labsize(small))  ylabel(, labsize(small) angle(horizontal)) ///
...
graph save "graph 32 COVID-19 daily hospital-related outcomes 2, Iran median scenario, IHME.gph", replace
// graph export "graph 32 COVID-19 daily hospital-related outcomes 2, Iran median scenario, IHME.pdf", replace
