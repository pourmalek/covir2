...
graph save "graph 23b COVID-19 daily cases, Iran, reference scenarios, 01dec2020 on, CI., w CF.gph", replace
...
graph save "graph 24b COVID-19 daily cases, Iran, reference scenarios, 01dec2020 on, CI, wo IMPE upper., w CF.gph", replace
...
(line TotDeaMeRaA03S02V07 date, sort lcolor(magenta)) /// 10 IMPE mean
(line TotDeaLoRaA03S02V07 date, sort lcolor(magenta) lpattern(dash)) /// 11 IMPE lower
(line TotDeaMeRaA05S00V0 date, sort lcolor(gold)) /// 12 LANL mean
(line TotDeaLoRaA05S00V0 date, sort lcolor(gold) lpattern(dash)) /// 13 LANL lower
(line TotDeaUpRaA05S00V0 date, sort lcolor(gold) lpattern(dash)) /// 14 LANL upper
...
* cases to deaths (CTD) (graph numbers start with 81)




* Day cases to deaths (CTD) all studies, all time, reference scenarios
...
* Day cases to deaths (CTD) all studies, 01apr2020 on, reference scenarios
...
(line DayCTDMeRaA04S00V02 date, sort lcolor(gold)) /// 5 "LANL"
(line DayCTDMeRaA05S00V02 date, sort lcolor(green)) /// 6 "SRIV"
if date > td(30mar2020) & date < td(02jul2021) ///
...
* Day cases to deaths (CTD) all studies, 01apr2020 on, 3 scenarios
...
(line DayDERMeRaA04S00V02 date, sort lcolor(gold)) /// 4 "LANL"
(line DayDERMeRaA05S00V02 date, sort lcolor(green)) /// 5 "SRIV"
if date > td(01mar2021) & date < td(02apr2021) ///
, xtitle(Date) xlabel(#19, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
...
twoway ///
(line DayCERMeRaA01S00V03 date, sort lcolor(red)) /// 1 "DELP"
...
twoway ///
(line TotCERMeRaA01S00V03 date, sort lcolor(red)) /// 1 "DELP"
...
if date > td(01mar2021) & date < td(02apr2021) ///
, xtitle(Date) xlabel(#19, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(0(2)14, format(%12.2fc) labsize(small))  ylabel(, labsize(small) angle(horizontal)) ///
ytitle(Total cases estimated to reprted) title("COVID-19 total cases estimated to reprted, Iran, reference scenarios", size(medium)) /// 
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) legend(region(lcolor(none))) legend(bexpand) ///
legend(order(1 "DELP" 2 "IHME" 3 "IMPE" 4 "LANL" 5 "SRIV") size(small) row(1))


graph save "graph 97 COVID-19 total cases estimated to reprted, Iran, reference scenarios, 01mar2021 on.gph", replace
graph export "graph 97 COVID-19 total cases estimated to reprted, Iran, reference scenarios, 01mar2021 on.pdf", replace
