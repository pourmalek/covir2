...
copy https://raw.githubusercontent.com/mrc-ide/global-lmic-reports/master/IRN/2021-03-19/projections.csv projections.csv
...
if date > td(31dec2019) & date < td(2jul2021) & date < td(2jul2021) ///
...
(line DayDeaMeRaA03S02V07 date, sort lcolor(black)) ///
if date > td(31dec2019) & date < td(2jul2021) ///
, xtitle(Date) xlabel(#19, format(%tdYY-NN-DD) labsize(small)) xlabel(, grid) xlabel(, grid) ///
xlabel(, angle(forty_five)) ylabel(, format(%9.0fc) labsize(small))  ylabel(, labsize(small) angle(horizontal)) ///
xscale(lwidth(vthin) lcolor(gray*.2)) yscale(lwidth(vthin) lcolor(gray*.2)) ///
